...
(function () {{
  ...
  async function ensureCsrfToken() {{
    ...
    const cookieToken = readCookie(config.csrfCookieName);
    if (cookieToken) {{
      return cookieToken;
    }}
  ...
  async function attachCsrf(headers) {{
    ...
    if (!csrfToken) {{
      throw new Error(
        `Missing CSRF cookie "${{config.csrfCookieName}}". Authenticate through authSDK first.`
      );
    }}
  ...
  window.__TWA_SWAGGER_REQUEST_INTERCEPTOR__ = async function (request) {{
    ...
    if (
      config.applyCsrfToUnsafeRequests &&
...
def build_swagger_ui_html(
    *,
    ...
    if _SWAGGER_UI_SCRIPT_MARKER not in html:
        raise RuntimeError(
            "FastAPI Swagger UI HTML did not contain the expected script marker."
        )
    if _SWAGGER_UI_INIT_MARKER not in html:
        raise RuntimeError(
            "FastAPI Swagger UI HTML did not contain the expected init marker."
        )
    ...
    return HTMLResponse(
        content=html, status_code=base_response.status_code, headers=headers
    )
