...
import type { EmployerProfile, EmployerProfileFormValues } from '../types/employer'
...
export function EmployerProfileForm({
  ...
  const [values, setValues] = useState<EmployerProfileFormValues>(() => toValues(profile))
  ...
  return (
    <form className="stack-md" onSubmit={(event) => {
      ...
      <div className="employer-form-grid">
        <Field label="Organization name"><input required value={values.org_name} onChange={(event) => setValues({ ...values, org_name: event.target.value })} /></Field>
        <Field label="Contact name"><input value={values.contact_name} onChange={(event) => setValues({ ...values, contact_name: event.target.value })} /></Field>
      ...
      <Button disabled={isSubmitting} type="submit">{isSubmitting ? 'Saving profile...' : 'Save employer profile'}</Button>
    </form>
  )
}
