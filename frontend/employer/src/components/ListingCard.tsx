...
import { formatChargeFlags, formatDate, formatTransitAccessibility, formatTransitRequirement } from '../lib/formatting'
import type { JobListing } from '../types/employer'

export function ListingCard({ listing, children }: { listing: JobListing; children?: ReactNode }) {
      ...
      <CardBody className="stack-md">
        <div className="cluster" style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div className="stack-sm">
            ...
            <p className="card-copy">{listing.city ?? 'City not set'}{listing.zip ? `, ${listing.zip}` : ''}</p>
          </div>
        ...
        <div className="listing-meta-grid">
          ...
          <p className="card-copy">{charges.length > 0 ? `Disqualifying: ${charges.join(', ')}` : 'No disqualifying charges set'}</p>
        </div>
        {listing.review_note ? <p className="listing-note">{listing.review_note}</p> : null}
