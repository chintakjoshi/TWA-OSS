...
test('staff can search auth users, inspect suspicious sessions, and revoke a session through step-up verification', async () => {
    ...
    requestAuthImpl: async (path, init) => {
      ...
      if (path === '/admin/users/user-1') {
        return {
          ...
          created_at: '2026-04-01T09:00:00Z',
          updated_at: '2026-04-17T09:00:00Z',
          active_session_count: sessions.filter((item) => !item.revoked_at)
            .length,
        }
      }
  ...
  expect(
    await within(revokeDialog).findByText(
  ...
  await waitFor(() => {
    expect(
      screen.getByText('Session revoked successfully.')
    ).toBeInTheDocument()
  })
...
test('staff can preview and execute a filter-based revoke sweep through separate step-up confirmations', async () => {
  const user = userEvent.setup()
  let sessions = [
    {
      session_id: 'session-1',
  ...
  const { client, spies } = createMockAuthClient({
    ...
    requestAuthImpl: async (path, init) => {
      ...
      if (path === '/auth/otp/verify/action') {
        ...
        const expectedCode = actionVerificationCount === 1 ? '111111' : '222222'
        expect(init?.body).toBe(
  ...
  await waitFor(() => {
    expect(screen.getByText('Revoked 1 filtered session.')).toBeInTheDocument()
  })
...
test('staff can load the global suspicious queue, page through it, filter it, and drill into another user workspace', async () => {
  const user = userEvent.setup()
  ...
  const { client } = createMockAuthClient({
    ...
    requestAuthImpl: async (path) => {
      ...
      if (
        path === '/admin/sessions/suspicious?cursor=queue-cursor-1&limit=50'
      ) {
        return {
          data: [
            {
  ...
  await user.click(
    screen.getByRole('button', { name: 'Load suspicious queue' })
  )
  ...
  expect(
    (await screen.findAllByText('Firefox on Linux')).length
  ).toBeGreaterThan(0)
}, 12000)

test('directory searches reconcile the workspace to the returned user and reset per-user session filters', async () => {
  const user = userEvent.setup()
  ...
  expect(
    screen.queryByText('Revoked Chrome on Windows')
