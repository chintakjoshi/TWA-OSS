...
export function AdminEmployersPage() {
    ...
    try {
      ...
      setError(
        nextError instanceof Error
          ? nextError.message
          : 'Unable to load employer data right now.'
      )
    } finally {
      setIsLoading(false)
    }
  }, [auth.requestTwa, queuePage, allPage, reviewFilter])
  ...
  async function handleReviewSave() {
    ...
    setIsSaving(true)
    try {
      await reviewEmployer(auth.requestTwa, selected.id, {
  ...
  return (
    <div className="page-frame stack-md admin-shell-page">
          ...
          >
            <div className="stack-sm">
              ...
              <h2 className="card-title">
                Review employer accounts and revisit previous decisions.
              </h2>
              <p className="card-copy">
                Rejected employers can be reassessed later, so the full list
                stays visible alongside the pending queue.
              </p>
            </div>
      ...
      {isLoading ? (
        <LoadingState title="Loading employer review queues..." />
      ) : null}
      {!isLoading &&
      error &&
      queueItems.length === 0 &&
            ...
            <CardBody className="stack-md">
              ...
              {queueItems.length === 0 ? (
                <EmptyState
                  ...
                  message="The employer approval queue is currently clear."
                />
              ) : (
                <DataTable
                  columns={[
            ...
            <CardBody className="stack-md">
              ...
              >
                <div className="stack-sm">
                  ...
                  <h2 className="card-title">
                    Review history stays visible for reassessment.
                  </h2>
                </div>
              ...
              {allItems.length === 0 ? (
                <EmptyState
                  ...
                  message="Try clearing the current filter or create more seed data."
                />
              ) : (
                <DataTable
                  columns={[
        ...
        {selected ? (
          <div className="stack-md">
            <div className="detail-grid">
              <div className="stack-sm">
                <p className="card-copy">
                  Contact: {selected.contact_name ?? 'No contact'}
                </p>
                <p className="card-copy">
                  Phone: {selected.phone ?? 'No phone'}
                </p>
                <p className="card-copy">
              ...
              <div className="stack-sm">
                <p className="card-copy">
                  Current status: {selected.review_status}
                </p>
                <p className="card-copy">
